    ...
    const fetchSummary = async () => {
        ...
        try {
            ...
            if (err.details?.status === 403) {
                setError("Acceso Restringido: Se requiere rol de ADMINISTRADOR para ver la contabilidad general.");
            } else {
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        {summary.income_by_method && (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {/* Cash Card */}
                                <div className="bg-white p-6 rounded-[2rem] shadow-xl shadow-slate-200 border border-slate-100 relative overflow-hidden group">
                                    <div className="absolute top-0 right-0 p-4">
                                        <Wallet className="w-8 h-8 text-amber-100 group-hover:text-amber-200 transition-colors" />
                                    </div>
                                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">💵 Ingresos Metálico</h3>
                                    <div className="space-y-4">
                                        <span className="text-3xl font-black text-amber-600 tracking-tighter">
                                            €{(summary.income_by_method.CASH?.total || 0).toFixed(2)}
                                        </span>
                                        <div className="pt-4 border-t border-slate-50 text-[10px] font-bold text-slate-500">
                                            <p>{summary.income_by_method.CASH?.count || 0} reservas</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white p-6 rounded-[2rem] shadow-xl shadow-slate-200 border border-slate-100 relative overflow-hidden group">
                                    <div className="absolute top-0 right-0 p-4">
                                        <CheckCircle2 className="w-8 h-8 text-blue-100 group-hover:text-blue-200 transition-colors" />
                                    </div>
                                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">💳 Ingresos Tarjeta / Bizum</h3>
                                    <div className="space-y-4">
                                        <span className="text-3xl font-black text-blue-600 tracking-tighter">
                                            €{(summary.income_by_method.CARD?.total || 0).toFixed(2)}
                                        </span>
                                        <div className="pt-4 border-t border-slate-50 text-[10px] font-bold text-slate-500">
                                            <p>{summary.income_by_method.CARD?.count || 0} reservas</p>
                                        </div>
                                    </div>
                                </div>

                                {/* Unknown Card */}
                                {(summary.income_by_method.UNKNOWN?.count || 0) > 0 && (
                                    <div className="bg-slate-50 p-6 rounded-[2rem] shadow-xl shadow-slate-200 border border-slate-200 relative overflow-hidden group">
                                        <div className="absolute top-0 right-0 p-4">
                                            <AlertCircle className="w-8 h-8 text-slate-200 group-hover:text-slate-300 transition-colors" />
                                        </div>
                                        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">❓ Sin Método Asignado</h3>
                                        <div className="space-y-4">
                                            <span className="text-3xl font-black text-slate-500 tracking-tighter">
                                                €{(summary.income_by_method.UNKNOWN?.total || 0).toFixed(2)}
                                            </span>
                                            <div className="pt-4 border-t border-slate-200 text-[10px] font-bold text-slate-500">
                                                <p>{summary.income_by_method.UNKNOWN?.count || 0} reservas</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
