    ...
    async function generateExport(format: "pdf" | "xlsx") {
        ...
        try {
            ...
            if (reportType === "summary") {
                const stats = await apiFetch<any>("/analytics/summary");
                reportData = stats;
            } else if (reportType === "invoice") {
                const now = new Date();
                const res = await apiFetch<any>(`/reports/billing?month=${now.getMonth() + 1}&year=${now.getFullYear()}`);
                reportData = {
                    items: res.invoices,
                    total_cents: res.summary.totalRevenue
                };
            } else if (reportType === "reservation") {
                const now = new Date();
                const from = now.toISOString().split('T')[0];
                const res = await apiFetch<any>("/reports/courts/summary", {
                    method: 'POST',
                    body: JSON.stringify({ from, to: from, courtId: 'all' })
                });
                reportData = {
                    items: res.details
                };
